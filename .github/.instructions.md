...
```lua
{
    ...
    alertX = number,          -- Legacy CENTER-relative X (only used to migrate to alertLeft/alertTop)
    alertY = number,          -- Legacy CENTER-relative Y (only used to migrate to alertLeft/alertTop)
    alertLeft = number,       -- Alert frame left edge (TOPLEFT anchored to UIParent BOTTOMLEFT)
    alertTop = number,        -- Alert frame top edge (TOPLEFT anchored to UIParent BOTTOMLEFT)
